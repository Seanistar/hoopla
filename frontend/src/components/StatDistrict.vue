<template>
  <v-container pt-2 mt-1>
    <v-layout row align-baseline v-if="$parent.window.width >= 900">
      ...
      <v-flex xs2 pl-3>
        <v-select label="지구선택" class="text-xs-center body-1" clearable hide-details
                  v-model="district" :items="districts" item-value="m_code" item-text="m_name"
        ></v-select>
      </v-flex>
      <v-flex xs2 pl-3>
        <v-select label="연도선택" class="text-xs-center body-1" clearable hide-details
                  v-model="year" :items="years"></v-select>
      </v-flex>
      <v-flex xs3 ml-3>
    ...
    <v-layout row align-baseline v-else-if="$parent.window.width >= 600">
      ...
      <v-flex xs3 ml-3 pl-2>
        <v-select label="지구선택" class="text-xs-center body-1" clearable
                  v-model="district" :items="districts" item-value="m_code" item-text="m_name"
        ></v-select>
      </v-flex>
      <v-flex xs3 ml-3 pl-2>
    ...
    <v-layout v-else>
      ...
      <v-flex xs6 ml-2>
        <v-select label="지구선택" class="text-xs-center body-1" clearable
                  v-model="district" :items="districts" item-value="m_code" item-text="m_name"></v-select>
      </v-flex>
      <v-flex xs6 ml-2>
...
export default {
  ...
  computed: {
    ...
    districts () {
      ...
      let rest = filter(uniqBy(this.areaCodes, 'm_code'), (o) => startsWith(o.a_code, `${this.area}-`))
  ...
  methods: {
    async _fecthData () {
      this.fetched = false
      const params = { district: this.district, year: this.year, type: this.type }
      await this.$store.dispatch(FETCH_STAT_DISTRICT, {params})
      this.$nextTick(() => { this._mapData() })
    },
    _mapData () {
      const qv = this.statDistrict
      if (!qv) return
      ...
      Object.keys(yg).forEach(k => {
        ...
        if (area) {
          let obj = {}; obj[`${area.m_name}-${area.s_name}`] = yg[k]
          this.items.push(obj)
        }
      })
    ...
    toExcel () {
      ...
      XLSX.writeFile(wb, 'stats_district.xlsx')
    }
  },
  filters: {
    units (x) {
