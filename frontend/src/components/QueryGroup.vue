    ...
    <v-data-table hide-actions class="elevation-1" :items="items">
      <template slot="headers" slot-scope="props">
        <tr>
          ...
          <th :colspan="grpCodes.length" class="body-2 font-weight-bold w-90">그룹 공부 현황</th>
        </tr>
        <tr>
          <th class="body-1 font-weight-medium w-10"
              v-for="item in grpCodes" :key="item.code">{{item.name|subject}}</th>
        </tr>
      </template>
      ...
      <template slot="items" slot-scope="props">
        <tr> <!--@click="selected = props.item" :style="{backgroundColor: (selected.id === props.item.id ? 'orange' : 'white')}">-->
          ...
          <td class="text-xs-center" v-for="(tr, i) in grpCodes" :key="i">{{ props.item|yearBy|counter(tr) }}</td>
        </tr>
      </template>
